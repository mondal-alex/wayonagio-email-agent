...
class TestLoad:
    def test_raises_when_rag_folder_ids_missing(self, monkeypatch):
        for var in (
            "KB_RAG_FOLDER_IDS",
            "KB_RAG_RECURSIVE",
        ...
        with pytest.raises(config.KBConfigError, match="KB_RAG_FOLDER_IDS"):
            config.load()

    def test_defaults_when_only_required_var_set(self, monkeypatch):
        for var in (
            "KB_RAG_RECURSIVE",
            "KB_INCLUDE_MIME_TYPES",
            "KB_EMBEDDING_MODEL",
            ...
            monkeypatch.delenv(var, raising=False)
        monkeypatch.setenv("KB_RAG_FOLDER_IDS", "1abc")
